...
printf "${YELLOW}[paper-review]${NC} Make sure you have downloaded the data.\n"
printf "${YELLOW}[paper-review]${NC} Run './download_data.sh' from this task directory if needed.\n\n"

python orchestration/orchestrator.py \
  --task_dir      ./tasks/paper-review \
  --max_gen       "${MAX_GEN}" \
  --max_turns     "${MAX_TURNS}" \
